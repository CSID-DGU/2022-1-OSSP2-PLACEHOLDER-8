...
def search_by_circle(id):
        ...
        if spectrum_dot_mode:
            ...
            for idx_1, pos in enumerate(position_bundle):
                ...
                for i in range(len(pos)):
                    plt.plot(
                        pos[i]['x'], pos[i]['y'], color=color[idx_1], marker='o', markersize=5
                    )
    ...
    for x in compare_db(
        user_match['Map_Name'],
        user_match['Match_Mode'],
        user_match['F_Circle_x'],
        user_match['F_Circle_y'],
        user_match['L_Circle_x'],
        user_match['L_Circle_y'],
    ):
    ...
    def plot_map_img(map_name=None, res_option='Low'):  # 맵 띄우기
        ...
        while k < len(circle_list):
            plt.gca().add_patch(
                patches.Circle(
                    (circle_list[k][1] / map_x, circle_list[k][2] / map_y),
                    circle_list[k][4] / map_x,
                    color='w',
                    fill=False,
                )
            )  # 첫번째 매치 원
        ...
        while k < len(search_circle_list):
            plt.gca().add_patch(
                patches.Circle(
                    (search_circle_list[k][1] / map_x, search_circle_list[k][2] / map_y),
                    search_circle_list[k][4] / map_x,
                    color=color[idx[1]],
                    fill=False,
                )
            )  # 첫번째 매치 원
